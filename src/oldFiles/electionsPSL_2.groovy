 ...
 * 	version		=	2.0
 *  comments    =	DQE done inside PSL
 *					propagation based on BelongedTo and Seed_words only
...
class electionsPSL
{
	...
	public electionsPSL(String inputFolder)
    {
        ...
        //membership rules
		model.add rule: (TWEETED(A,T) & CONTAINS(T,W) & BELONGED_TO(W,G) & POSITIVE_SENTIMENT(T)) >> IS_MEMBER(A,G), weight: 1.0
        model.add rule: (TWEETED(A,T) & CONTAINS(T,W) & BELONGED_TO(W,G) & NEGATIVE_SENTIMENT(T)) >> ~IS_MEMBER(A,G), weight: 1.0
        model.add rule: (TWEETED(A,T) & CONTAINS(T,W) & SEED_WORD(W,G) & POSITIVE_SENTIMENT(T)) >> IS_MEMBER(A,G), weight: 3.0
        model.add rule: (TWEETED(A,T) & CONTAINS(T,W) & SEED_WORD(W,G) & NEGATIVE_SENTIMENT(T)) >> ~IS_MEMBER(A,G), weight: 3.0

        //propagating membership through social graph
        //retweets
        model.add rule: (IS_MEMBER(B,G) & TWEETED(A,T) & RETWEET_OF(T,B) & CONTAINS(T,W) & SEED_WORD(W,G)) >> IS_MEMBER(A,G), weight: 3.0
        model.add rule: (IS_MEMBER(B,G) & TWEETED(B,T) & RETWEET_OF(T,A) & CONTAINS(T,W) & SEED_WORD(W,G)) >> IS_MEMBER(A,G), weight: 3.0
        model.add rule: (IS_MEMBER(B,G) & TWEETED(A,T) & RETWEET_OF(T,B) & CONTAINS(T,W) & BELONGED_TO(W,G)) >> IS_MEMBER(A,G), weight: 1.0
        model.add rule: (IS_MEMBER(B,G) & TWEETED(B,T) & RETWEET_OF(T,A) & CONTAINS(T,W) & BELONGED_TO(W,G)) >> IS_MEMBER(A,G), weight: 1.0
        //mentions
	    model.add rule: (IS_MEMBER(A,G) & TWEETED(B,T) & MENTIONS(T,A) & CONTAINS(T,W) & SEED_WORD(W,G) & POSITIVE_SENTIMENT(T)) >> IS_MEMBER(B,G), weight: 2.0
		model.add rule: (IS_MEMBER(A,G) & TWEETED(A,T) & MENTIONS(T,B) & CONTAINS(T,W) & SEED_WORD(W,G) & POSITIVE_SENTIMENT(T)) >> IS_MEMBER(B,G), weight: 2.0
        model.add rule: (IS_MEMBER(A,G) & TWEETED(B,T) & MENTIONS(T,A) & CONTAINS(T,W) & BELONGED_TO(W,G) & POSITIVE_SENTIMENT(T)) >> IS_MEMBER(B,G), weight: 1.0
		model.add rule: (IS_MEMBER(A,G) & TWEETED(A,T) & MENTIONS(T,B) & CONTAINS(T,W) & BELONGED_TO(W,G) & POSITIVE_SENTIMENT(T)) >> IS_MEMBER(B,G), weight: 1.0

        //propagating word preferences
        //single tweet co-occurance propagation
        model.add rule: (TWEETED(A,T) & CONTAINS(T,W1) & BELONGED_TO(W1,G) & CONTAINS(T,W2) & (W1-W2) & POSITIVE_SENTIMENT(T)) >> BELONGS_TO(W2,G), weight: 1.0
        model.add rule: (TWEETED(A,T) & CONTAINS(T,W1) & BELONGED_TO(W1,G) & CONTAINS(T,W2) & (W1-W2) & NEGATIVE_SENTIMENT(T)) >> ~BELONGS_TO(W2,G), weight: 1.0
        model.add rule: (TWEETED(A,T) & CONTAINS(T,W1) & SEED_WORD(W1,G) & CONTAINS(T,W2) & (W1-W2) & POSITIVE_SENTIMENT(T)) >> BELONGS_TO(W2,G), weight: 2.0
        model.add rule: (TWEETED(A,T) & CONTAINS(T,W1) & SEED_WORD(W1,G) & CONTAINS(T,W2) & (W1-W2) & NEGATIVE_SENTIMENT(T)) >> ~BELONGS_TO(W2,G), weight: 2.0
        //two members using the same word
        model.add rule: (IS_MEMBER(A,G) & IS_MEMBER(B,G) & TWEETED(A,T1) & CONTAINS(T1,W) & TWEETED(B,T2) & CONTAINS(T2,W) & (T1-T2) & (A-B) & POSITIVE_SENTIMENT(T1) & POSITIVE_SENTIMENT(T2)) >> BELONGS_TO(W,G), weight: 2.0
        model.add rule: (IS_MEMBER(A,G) & IS_MEMBER(B,G) & TWEETED(A,T1) & CONTAINS(T1,W) & TWEETED(B,T2) & CONTAINS(T2,W) & (T1-T2) & (A-B) & NEGATIVE_SENTIMENT(T1) & NEGATIVE_SENTIMENT(T2)) >> ~BELONGS_TO(W,G), weight: 2.0
