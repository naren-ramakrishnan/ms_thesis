...
class electionsPSL{
	...
	public electionsPSL(String inputFolder){
        ...
        model.add predicate: "POSITIVE", types: [ArgumentType.UniqueID]
		model.add predicate: "NEGATIVE", types: [ArgumentType.UniqueID]
        model.add predicate: "SCORE", types:[ArgumentType.UniqueID]
        ...
        model.add rule: ~BELONGS_TO(W,G), weight: 0.05
        model.add rule: ~IS_MEMBER(U,G), weight: 0.05

        log.info("defining rules")
        //previous rounds values are just priors should be over come in light of new evidence
        ...
        //membership rules
        model.add rule: (TWEETED(A,T) & CONTAINS(T,W) & BELONGED_TO(W,G) & POSITIVE(T) & SCORE(T)) >> IS_MEMBER(A,G), weight: 0.5
        model.add rule: (TWEETED(A,T) & CONTAINS(T,W) & BELONGED_TO(W,G) & NEGATIVE(T) & SCORE(T)) >> ~IS_MEMBER(A,G), weight: 0.5
        model.add rule: (TWEETED(A,T) & CONTAINS(T,W) & BELONGS_TO(W,G) & POSITIVE(T) & SCORE(T)) >> IS_MEMBER(A,G), weight: 1.0
        model.add rule: (TWEETED(A,T) & CONTAINS(T,W) & BELONGS_TO(W,G) & NEGATIVE(T) & SCORE(T)) >> ~IS_MEMBER(A,G), weight: 1.0

        //propagating membership through social interactions
        //retweets
        model.add rule: (IS_MEMBER(B,G) & TWEETED(A,T) & RETWEET_OF(T,B)) >> IS_MEMBER(A,G), weight: 1.0
        model.add rule: (IS_MEMBER(A,G) & TWEETED(A,T) & RETWEET_OF(T,B)) >> IS_MEMBER(B,G), weight: 1.0
        //mentions
        model.add rule: (IS_MEMBER(A,G) & TWEETED(A,T) & MENTIONS(T,B) & POSITIVE(T) & SCORE(T)) >> IS_MEMBER(B,G), weight: 0.5
		model.add rule: (IS_MEMBER(A,G) & TWEETED(B,T) & MENTIONS(T,A) & NEGATIVE(T) & SCORE(T)) >> ~IS_MEMBER(B,G), weight: 0.5

        //propagating word preferences
        model.add rule: (IS_MEMBER(A,G) & TWEETED(A,T) & CONTAINS(T,W) & POSITIVE(T) & SCORE(T)) >> BELONGS_TO(W,G), weight: 1.0
        model.add rule: (IS_MEMBER(A,G) & TWEETED(A,T) & CONTAINS(T,W) & NEGATIVE(T) & SCORE(T)) >> ~BELONGS_TO(W,G), weight: 1.0
        //co-occurance
        model.add rule: (CONTAINS(T,W1) & BELONGED_TO(W1,G) & CONTAINS(T,W2) & (W1-W2) & POSITIVE(T) & SCORE(T)) >> BELONGS_TO(W2,G), weight: 0.5
        model.add rule: (CONTAINS(T,W1) & BELONGED_TO(W1,G) & CONTAINS(T,W2) & (W1-W2) & NEGATIVE(T) & SCORE(T)) >> ~BELONGS_TO(W2,G), weight: 0.5
    ...
    private void loadSentimentPredicate(String inputFile, Database db){
        ...
        int maxSent = 100
        double coeff = 5.0
        double[] truthValues = new double[101]
        for (int sent=minSent; sent<=maxSent; sent++)
             truthValues[sent]= (1 - Math.exp(-coeff * (sent)))

        while (reader.ready()){
            String line = reader.readLine()
            String[] splits = line.split(',')
            int sentiment = Integer.parseInt(splits[1])
            int index = Math.abs(sentiment)

            if (sentiment == 0){
                insertAtom(0.5, db, POSITIVE, splits[0])
                insertAtom(0.5, db, NEGATIVE, splits[0])
                insertAtom(truthValues[index], db, SCORE, splits[0])
            }
            else if(sentiment > 0){
				insertAtom(1.0, db, POSITIVE, splits[0])
                insertAtom(truthValues[index], db, SCORE, splits[0])
            }
            else{
                insertAtom(1.0, db, NEGATIVE, splits[0])
                insertAtom(truthValues[index], db, SCORE, splits[0])
            }
		}
        reader.close()
    ...
    public void inferAndPrint(String outputFolder){
        ...
        //inference
        Database db = data.getDatabase(inferredPartition, [TWEETED, MENTIONS, RETWEET_OF, CONTAINS, POSITIVE, SCORE, NEGATIVE, WAS_MEMBER, BELONGED_TO] as Set, observedPartition)
