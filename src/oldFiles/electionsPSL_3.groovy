 ...
 *                  SEED_WORDS are loaded into BELONGED_TO
 *                  
 */
...
class electionsPSL
{
	...
	public electionsPSL(String inputFolder)
    {
        ...
        //model.add rule: SEED_WORD(W,G) >> BELONGED_TO(W,G), weight: 2.0
        ...
        //consider removing the contains and belongs(ed)_TO predicat
        //retweets
        model.add rule: (IS_MEMBER(B,G) & TWEETED(A,T) & RETWEET_OF(T,B) & CONTAINS(T,W) & BELONGED_TO(W,G)) >> IS_MEMBER(A,G), weight: 1.0
        model.add rule: (IS_MEMBER(B,G) & TWEETED(A,T) & RETWEET_OF(T,B) & CONTAINS(T,W) & BELONGED_TO(W,G)) >> IS_MEMBER(A,G), weight: 1.0
        model.add rule: (IS_MEMBER(B,G) & TWEETED(B,T) & RETWEET_OF(T,A) & CONTAINS(T,W) & BELONGS_TO(W,G)) >> IS_MEMBER(A,G), weight: 1.0
        model.add rule: (IS_MEMBER(B,G) & TWEETED(B,T) & RETWEET_OF(T,A) & CONTAINS(T,W) & BELONGS_TO(W,G)) >> IS_MEMBER(A,G), weight: 1.0
        //mentions
        ...
        model.add rule: (IS_MEMBER(A,G) & TWEETED(B,T) & MENTIONS(T,A) & CONTAINS(T,W) & BELONGS_TO(W,G) & POSITIVE_SENTIMENT(T)) >> IS_MEMBER(B,G), weight: 1.0
		model.add rule: (IS_MEMBER(A,G) & TWEETED(A,T) & MENTIONS(T,B) & CONTAINS(T,W) & BELONGS_TO(W,G) & POSITIVE_SENTIMENT(T)) >> IS_MEMBER(B,G), weight: 1.0
        ...
        model.add rule: (TWEETED(A,T) & CONTAINS(T,W1) & BELONGED_TO(W1,G) & CONTAINS(T,W2) & (W1-W2) & POSITIVE_SENTIMENT(T)) >> BELONGS_TO(W2,G), weight: 2.0
        model.add rule: (TWEETED(A,T) & CONTAINS(T,W1) & BELONGED_TO(W1,G) & CONTAINS(T,W2) & (W1-W2) & NEGATIVE_SENTIMENT(T)) >> ~BELONGS_TO(W2,G), weight: 2.0
	...
	private void loadSeedWordsPredicate(String seedWordsFile, Database db)
	{
		...
		while(reader.ready())
		{
			...
			insertAtom(1.0, db, BELONGED_TO, splits[0], splits[1])
		}
		log.info("SEED_WORD predicate loaded into BELONGED_TO")
		reader.close()
		decoder.close()
		fis.close()
    }
