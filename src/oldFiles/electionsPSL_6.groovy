...
class electionsPSL{
	...
	public electionsPSL(String inputFolder){
        ...
        model.add rule: ~BELONGS_TO(W,G), weight: 0.1
        model.add rule: ~IS_MEMBER(U,G), weight: 0.1
        ...
        model.add rule: (IS_MEMBER(A,G) & TWEETED(A,T) & CONTAINS(T,W1) & BELONGS_TO(W1,G) & CONTAINS(T,W2) & (W1-W2) & POSITIVE_SENTIMENT(T)) >> BELONGS_TO(W2,G), weight: 1.0
        model.add rule: (IS_MEMBER(A,G) & TWEETED(A,T) & CONTAINS(T,W1) & BELONGS_TO(W1,G) & CONTAINS(T,W2) & (W1-W2) & NEGATIVE_SENTIMENT(T)) >> ~BELONGS_TO(W2,G), weight: 1.0
        //two members using the same word
        model.add rule: (IS_MEMBER(A,G) & TWEETED(A,T1) & CONTAINS(T1,W) & IS_MEMBER(B,G) & TWEETED(B,T2) & CONTAINS(T2,W) & (A-B) & POSITIVE_SENTIMENT(T1) & POSITIVE_SENTIMENT(T2)) >> BELONGS_TO(W,G), weight: 1.0
        //model.add rule: (IS_MEMBER(A,G) & TWEETED(A,T1) & CONTAINS(T1,W) & IS_MEMBER(B,G) & TWEETED(B,T2) & CONTAINS(T2,W) & (A-B) & NEGATIVE_SENTIMENT(T1) & NEGATIVE_SENTIMENT(T2)) >> ~BELONGS_TO(W,G), weight: 1.0
