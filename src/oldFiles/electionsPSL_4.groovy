 ...
 * 	version		=	4.0
 *  comments    =   Likes/Hates ---> belongsTo/~belongsTo
 *                  SeedWords --> belongedTo
 *                  POS_SENT, NEG_SENT directly used as squashing function
 *                  RETWEETS and MENTIONS separate Methods
 */
...
class electionsPSL{
	...
	public electionsPSL(String inputFolder){
        ...
        model.add rule: (IS_MEMBER(B,G) & TWEETED(B,T) & RETWEET_OF(T,A)) >> IS_MEMBER(A,G), weight: 0.5
        //mentions
		model.add rule: (IS_MEMBER(A,G) & TWEETED(A,T) & MENTIONS(T,B) & POSITIVE_SENTIMENT(T)) >> IS_MEMBER(B,G), weight: 0.5
		model.add rule: (IS_MEMBER(A,G) & TWEETED(B,T) & MENTIONS(T,A) & POSITIVE_SENTIMENT(T)) >> IS_MEMBER(B,G), weight: 0.5
