...
m.add Prior.Simple, on: groupLikesGroup, weight: 0.00001
m.add Prior.Simple, on: groupHatesGroup, weight: 0.00001
...
for (int i = 0; i < 10; i++) 
	insert.loadFromFile("lda/10seed" + i + ".txt")

insert = data.getInserter(groupLikesHashtag, 0)
insert.insert("#chavez", "Chavez_Supporters")
insert.insert("#capriles", "Capriles_Supporters")
...
writer = new FileAtomPrintStream("isMemberLDA.txt")
inference.printAtoms(isMember, writer, false)

writer = new FileAtomPrintStream("groupLikesHashtagLDA.txt")
inference.printAtoms(groupLikesHashtag, writer, false)
writer = new FileAtomPrintStream("groupHatesHashtagLDA.txt")
inference.printAtoms(groupHatesHashtag, writer, false)
writer = new FileAtomPrintStream("groupLikesGroupLDA.txt")
inference.printAtoms(groupLikesGroup, writer, false)
writer = new FileAtomPrintStream("groupHatesGroupLDA.txt")
